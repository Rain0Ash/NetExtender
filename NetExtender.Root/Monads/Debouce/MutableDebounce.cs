...
namespace NetExtender.Types.Monads
{
    ...
    public class NotifyDebounce<T> : MutableDebounce<T>
    {
        ...
        public sealed override event PropertyChangingEventHandler? PropertyChanging;
        ...
        protected internal override Debounce<T> Internal
        {
            ...
            set
            {
                ...
                if (@internal)
                {
                    ...
                    changing?.Invoke(this, NotifyUtilities.Changing.Time);
                    changing?.Invoke(this, NotifyUtilities.Changing.SetTime);
                }

                if (delay)
                {
                    changing?.Invoke(this, NotifyUtilities.Changing.Delay);
                }

                if (@internal || delay)
                {
                    changing?.Invoke(this, new PropertyChanging(nameof(IsDebounce)));
                }
                ...
                if (@internal)
                {
                    ...
                    changed?.Invoke(this, NotifyUtilities.Changed.Time);
                    changed?.Invoke(this, NotifyUtilities.Changed.SetTime);
                }

                if (delay)
                {
                    changed?.Invoke(this, NotifyUtilities.Changed.Delay);
                }

                if (@internal || delay)
                {
                    changed?.Invoke(this, new PropertyChanged(nameof(IsDebounce)));
                }
            }
        }
    ...
    public class MutableDebounce<T> : IDebounce<T>, IDebounceEquality<T, Debounce<T>>, IDebounceEquality<T, MutableDebounce<T>>, ICloneable<Debounce<T>>, ICloneable<MutableDebounce<T>>, ISerializable, INotifyProperty
    {
        ...
        {
            return _internal.ReferenceEquals(other);
        }
        ...
        public Boolean ReferenceEquals(MutableDebounce<T>? other)
        {
            return other is not null && _internal.ReferenceEquals(other._internal);
        }
