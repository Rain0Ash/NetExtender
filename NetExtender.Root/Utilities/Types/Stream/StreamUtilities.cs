...
namespace NetExtender.Utilities.Types
{
    public static class StreamUtilities
    {
        ...
        {
            ReadAtLeastCore(stream, buffer, buffer.Length, true);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void ReadExactly(this Stream stream, Byte[] buffer, Int32 offset, Int32 count)
        {
            ReadAtLeastCore(stream, buffer.AsSpan(offset, count), count, true);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static async ValueTask ReadExactlyAsync(this Stream stream, Memory<Byte> buffer, CancellationToken token = default)
        {
            await ReadAtLeastAsyncCore(stream, buffer, buffer.Length, true, token);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static async ValueTask ReadExactlyAsync(this Stream stream, Byte[] buffer, Int32 offset, Int32 count, CancellationToken token = default)
        {
            await ReadAtLeastAsyncCore(stream, buffer.AsMemory(offset, count), count, true, token);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Int32 ReadAtLeast(this Stream stream, Span<Byte> buffer, Int32 minimum, Boolean @throw = true)
        {
            return ReadAtLeastCore(stream, buffer, minimum, @throw);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ValueTask<Int32> ReadAtLeastAsync(this Stream stream, Memory<Byte> buffer, Int32 minimum, Boolean @throw = true, CancellationToken token = default)
        {
            return ReadAtLeastAsyncCore(stream, buffer, minimum, @throw, token);
        }

        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        private static Int32 ReadAtLeastCore(Stream stream, Span<Byte> buffer, Int32 minimum, Boolean @throw)
        {
            Int32 start;
            Int32 read;

            for (start = 0; start < minimum; start += read)
            {
                read = stream.Read(buffer.Slice(start));

                if (read <= 0)
                {
                    return !@throw ? start : throw new StreamReadBeyondEOFException<Stream>();
                }
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        private static async ValueTask<Int32> ReadAtLeastAsyncCore(Stream stream, Memory<Byte> buffer, Int32 minimum, Boolean @throw, CancellationToken token)
        {
            Int32 start;
            Int32 read;
            ...
            for (start = 0; start < minimum; start += read)
            {
                read = await stream.ReadAsync(buffer.Slice(start), token).ConfigureAwait(false);
