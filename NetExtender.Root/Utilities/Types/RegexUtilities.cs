...
namespace NetExtender.Utilities.Types
{
    public static class RegexUtilities
    {
        ...
        public static IEnumerable<String> GetCaptures(this MatchCollection matches)
        {
            ...
            return matches
                .SelectMany(static match => match.Groups.OfType<Group>())
        ...
        public static IDictionary<String, IList<String>> MatchNamedCaptures(this MatchCollection matches, IEnumerable<String?> groups, Boolean nogroup)
        {
            ...
            static IEnumerable<T> WhereIfNot<T>(IEnumerable<T> source, Func<T, Boolean> predicate, Boolean condition)
            {
                return condition ? EnumerableBaseUtilities.WhereNot(source, predicate) : source;
            }

            Dictionary<String, IList<String>> captures = new Dictionary<String, IList<String>>(matches.Count);

            foreach (String groupname in WhereIfNot(EnumerableBaseUtilities.WhereNotNull(groups), MathUtilities.IsInt32, nogroup))
            {
                foreach (Match match in matches)
                {
                    ...
                    DictionaryBaseUtilities.GetOrAdd(captures, groupname, static () => new List<String>(8)).Add(group.Value);
                }
            }
