...
namespace NetExtender.Utilities.Types
{
    ...
    public static class ExceptionUtilities
    {
        ...
        {
            return array is not null ? TypeUtilities.New<AggregateException, String?, Exception[], Boolean>().Invoke(message, array, false) : null;
        }

        [StackTraceHidden]
        [MethodImpl(MethodImplOptions.NoInlining)]
        [return: NotNullIfNotNull("exception")]
        public static TException? Throwable<TException>(this TException? exception) where TException : Exception
        {
            if (exception is not null && exception.StackTrace is null)
            {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        [return: NotNullIfNotNull("exception")]
        public static TException? SetStackTrace<TException>(this TException? exception) where TException : Exception
        {
        ...
        private static String ToDiagnosticString(this Exception source, StringBuilder? builder)
        {
            ...
            for (Exception? exception = source; exception is not null; exception = exception?.InnerException)
            {
                ...
                switch (exception)
                {
                    ...
                    {
                        foreach (Exception inner in EnumerableBaseUtilities.WhereNotNull(reflection.LoaderExceptions))
                        {
        ...
        public static void Handle(this AggregateException exception, Func<Exception, Boolean> predicate, Boolean intact)
        {
            ...
            if (HandleRecursively(exception, predicate) is { } @throw)
            {
                throw @throw;
            }
        }
        ...
        private static AggregateException? HandleRecursively(AggregateException exception, Func<Exception, Boolean> predicate)
        {
            ...
            foreach (Exception inner in exception.InnerExceptions)
            {
                ...
                {
                    if (HandleRecursively(aggregate, predicate) is { } result)
                    {
