...
namespace NetExtender.Utilities.Types
{
    public static class LinkedListNodeUtilities
    {
        public static ImmutableDictionary<LinkedList<T>, ImmutableArray<LinkedListNode<T>>> UniqueSort<T>(this IEnumerable<LinkedListNode<T>> source)
        {
            ...
            ConcurrentHashSet<LinkedListNode<T>> unique = new ConcurrentHashSet<LinkedListNode<T>>(EnumerableBaseUtilities.WhereNotNull(source));
            Dictionary<LinkedList<T>, List<LinkedListNode<T>>?> group = unique.GroupBy(static node => node.List)
                .Where(static pair => pair.Key is not null)
                .ToDictionary(static pair => pair.Key!, static pair => new List<LinkedListNode<T>?>(pair))!;
            ...
            return group.Where(static item => item.Value is not null).ToImmutableDictionary(static pair => pair.Key, static pair => pair.Value!.ToImmutableArray());
        }
    ...
    public static class LinkedListNodesMarshal
    {
        private static class Storage<TList, TNode> where TList : class, ILinkedContainer where TNode : class, ILinkedNode
        {
            ...
            private static Expression<Func<TNode, TList>>? Factory(Type type)
            {
                ...
                PropertyInfo? property = type.GetProperties(binding).Where(static property => property.PropertyType.IsAssignableTo(typeof(ILinkedContainer))).OrderBy(static property => property.Name switch
                {
                ...
                FieldInfo? field = type.GetFields(binding).Where(static field => field.FieldType.IsAssignableTo(typeof(ILinkedContainer))).OrderBy(static field => field.Name switch
                {
                    "_list" => 0,
                    "_container" => 1,
                    _ => 2
