    ...
    public static partial class EnumUtilities
    {
        ...
        private static class ValuesStorage<T> where T : unmanaged, Enum
        {
            ...
            private static void Initialize(IEnumerable<T>? source)
            {
                ...
                ImmutableDictionary<T, Decimal> @decimal = values.ToImmutableDictionary(static value => value, static value => ((IConvertible) value).ToDecimal(null));
                (T? minimum, T? maximum) = values.Length > 0 ? EnumerableBaseUtilities.MinMax(values) : (default(T?), default(T?));
        ...
        private static class DescriptionStorage<T> where T : unmanaged, Enum
        {
            ...
            private static void Initialize(IEnumerable<T>? source)
            {
                ...
                Values = source.Select(Convert).Where(static value => value.Value is not null).ToImmutableDictionary()!;
            }
        ...
        private static class UnderlyingOperationStorage<T> where T : unmanaged, Enum
        {
            ...
            private static void Initialize()
            {
                T minimum = Minimum<T>();
                T maximum = Maximum<T>();
                ImmutableArray<EnumMember<T>> distinct = MembersStorage<T>.Members.OrderBy(static member => member.Value).Distinct(EnumMember<T>.ValueComparer).ToImmutableArray();
        ...
        internal static class EnumStorage<T> where T : unmanaged, Enum
        {
            ...
            private static void Initialize(IEnumerable<Enum<T>>? source)
            {
                ...
                ImmutableDictionary<T, ImmutableDictionary<LocalizationIdentifier, Enum<T>>> identifiers = values.ToImmutableDictionary(static value => value.Id, static value => DescriptionToEnumStorage<T>.Get().ToImmutableDictionary(static identifier => identifier, identifier => Enum<T>.Create(value, identifier)));
                ImmutableDictionary<LocalizationIdentifier, ImmutableSortedSet<Enum<T>>> grouping = identifiers.SelectMany(static pair => pair.Value).GroupBy(static pair => pair.Key, static pair => pair.Value).ToImmutableDictionary(static group => group.Key, static group => group.Select(static value => value).ToImmutableSortedSet());
            ...
            private static class Type<TEnum> where TEnum : Enum<T, TEnum>, new()
            {
                ...
                private static void Initialize(IEnumerable<TEnum>? source)
                {
                    ...
                    ImmutableDictionary<T, ImmutableDictionary<LocalizationIdentifier, TEnum>> identifiers = values.ToImmutableDictionary(static value => value.Id, static value => DescriptionToEnumStorage<T>.Get().ToImmutableDictionary(static identifier => identifier, identifier => Enum<T>.Create<TEnum>(value, identifier)));
                    ImmutableDictionary<LocalizationIdentifier, ImmutableSortedSet<TEnum>> grouping = identifiers.SelectMany(static pair => pair.Value).GroupBy(static pair => pair.Key, static pair => pair.Value).ToImmutableDictionary(static group => group.Key, static group => group.Select(static value => value).ToImmutableSortedSet());
        ...
        {
            private static ConcurrentDictionary<Type, EnumSynchronizationMember> Enums { get; set; } = new ConcurrentDictionary<Type, EnumSynchronizationMember>();
            ...
            {
                Initialize(assembly.GetSafeTypesUnsafe());
            }

            internal static void Initialize(IEnumerable<Type> types)
            {
                static Boolean Predicate(Type type)
                {
                    return type.HasAttribute<EnumSynchronizeAttribute>(false);
                }

                Parallel.ForEach(types, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount }, static type =>
                {
                    if (Predicate(type) && Member(type) is { Value: not null } member)
                    {
                        Enums[member.Key] = member.Value;
                    }
                });
            ...
            private static KeyValuePair<Type, EnumSynchronizationMember?> Member(Type type)
            {
                ...
                if (AttributeUtilities.GetCustomAttribute<EnumSynchronizeAttribute>(type) is not { } attribute || type.GetGenericArguments(typeof(Enum<,>)) is not { Length: 2 } generic)
                {
                    return new KeyValuePair<Type, EnumSynchronizationMember?>(type, null);
                }
            ...
            internal static void Reset(IEnumerable<Assembly> assemblies)
            {
                lock (Enums)
                {
                    Enums.Clear();
                    Initialize(assemblies.GetTypes());
                }
            }
