...
namespace NetExtender.Utilities.Core
{
    ...
    public static class ReactiveUtilities
    {
        ...
        {
            assembly = AppDomain.CurrentDomain.GetAssemblies().FirstOrDefault(static assembly => assembly.GetName().Name == nameof(ReactiveUI));
            reactive = default;
            raise = default;
        ...
        private record ReactiveObjectHandler
        {
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public static Boolean Handle<TSource, TDestination>(TSource source, ref TDestination field, NetExtender.Utilities.Core.RaiseAndSetIfChangedHandler<TSource, TDestination> factory, String? property, [MaybeNullWhen(false)] out TDestination result) where TSource : class
            {
                if (source is null)
                {
                    throw new ArgumentNullException(nameof(source));
            ...
            private static Action<Object?, String?>? CreateHandler<TDelegate, TArgs>(Type type) where TDelegate : Delegate
            {
                ...
                switch (fields.Length)
                {
                    ...
                    default:
                        ...
                        @event = name is not null ? fields.FirstOrDefault(field => MemberInfoUtilities.HasName(field, name)) : null;
                        break;
                }
