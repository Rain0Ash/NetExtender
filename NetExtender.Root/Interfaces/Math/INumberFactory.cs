            ...
            {
                {
                    ...
                    Set(in set.CreateChecked) = INetExtenderOperator.Initialize(this, nameof(CreateChecked), parameters);
                    Set(in set.CreateSaturating) = INetExtenderOperator.Initialize(this, nameof(CreateSaturating), parameters);
                    Set(in set.CreateTruncating) = INetExtenderOperator.Initialize(this, nameof(CreateTruncating), parameters);
                }

                {
                    Type[] parameters = { Type.MakeGenericMethodParameter(0), typeof(TSelf).MakeByRefType() };

                    if (Set(in set.TryConvertFromChecked, INetExtenderOperator.Initialize(this, nameof(TryConvertFromChecked), parameters)) is null)
                    {
                        yield return Exception<TryConverter<Any.Value, TSelf>>(this, TryConvertFromChecked);
                    }

                    if (Set(in set.TryConvertFromSaturating, INetExtenderOperator.Initialize(this, nameof(TryConvertFromSaturating), parameters)) is null)
                    {
                        yield return Exception<TryConverter<Any.Value, TSelf>>(this, TryConvertFromSaturating);
                    }

                    if (Set(in set.TryConvertFromTruncating, INetExtenderOperator.Initialize(this, nameof(TryConvertFromTruncating), parameters)) is null)
                    {
...

                {
                    ...
                    if (Set(in set.TryConvertToChecked, INetExtenderOperator.Initialize(this, nameof(TryConvertToChecked), parameters)) is null)
                    {
                        yield return Exception<TryConverter<TSelf, Any.Value>>(this, TryConvertToChecked);
                    }

                    if (Set(in set.TryConvertToSaturating, INetExtenderOperator.Initialize(this, nameof(TryConvertToSaturating), parameters)) is null)
                    {
                        yield return Exception<TryConverter<TSelf, Any.Value>>(this, TryConvertToSaturating);
                    }

                    if (Set(in set.TryConvertToTruncating, INetExtenderOperator.Initialize(this, nameof(TryConvertToTruncating), parameters)) is null)
                    {
