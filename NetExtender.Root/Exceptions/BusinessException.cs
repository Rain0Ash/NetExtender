...
{
    [Serializable]
    public class BusinessException<T> : BusinessException, IUnsafeBusinessException<T>
    {
        ...
        IBusinessExceptionInfo<T> IBusinessException<T>.Info
        {
            get
            {
                return Info;
        ...
        IBusinessExceptionInfo<T> IBusinessException<T>.Business
        {
            get
            {
                return Business;
        ...
        {
            return new BusinessInfo(Id, RayId, Message, Identity, Code, Description, When, RetryAfter, Status, inner, ToBusinessInfoReason(include)) { Business = true, Include = include, Data = Data };
        }

#if NET7_0_OR_GREATER
        [System.Text.Json.Serialization.JsonDerivedType(typeof(BusinessException.BusinessInfo))]
#endif
        public new record BusinessInfo : BusinessException.BusinessInfo, IBusinessExceptionInfo<T>
        {
            ...
            public BusinessInfo(Guid id, RayIdContext context, String? message, String? identity, T code, String? description, DateTime? when, TimeSpan? retry, HttpStatusCode? status, ImmutableList<BusinessException.BusinessInfo>? reason)
                : base(id, context, message, identity, description, when, retry, status, reason)
            {
                Code = code;
            }

            public BusinessInfo(Guid id, RayIdContext context, String? message, String? identity, T code, String? description, DateTime? when, TimeSpan? retry, HttpStatusCode? status, BusinessException.BusinessInfo? inner, ImmutableList<BusinessException.BusinessInfo>? reason)
                : base(id, context, message, identity, description, when, retry, status, inner, reason)
            {
                Code = code;
            }
    ...
    public class BusinessException : AggregateException, IUnsafeBusinessException, IUnsafeTraceException
    {
        ...
        public static implicit operator Task<BusinessResult>(BusinessException? value)
        {
            return value is not null ? Task.FromResult<BusinessResult>(value) : TaskUtilities<BusinessResult>.Default;
        }

        public static implicit operator ValueTask<BusinessResult>(BusinessException? value)
        {
            return value is not null ? ValueTask.FromResult<BusinessResult>(value) : ValueTaskUtilities<BusinessResult>.Default;
        }

        private static ConcurrentDictionary<Type, String> AutoNameStorage { get; } = new ConcurrentDictionary<Type, String>();
        private static Assembly Assembly { get; } = typeof(BusinessException).Assembly;
        public static Boolean IsAutoName { get; set; } = true;

        protected virtual Boolean AutoName
        {
            get
            {
                return IsAutoName;
        ...
        RayIdContext IUnsafeTraceException.RayId
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
        ...
        public virtual String? Identity
        {
            ...
            get
            {
                if (Name is not null || !AutoName)
                {
                    return Name;
                }

                return AutoNameStorage.GetOrAdd(GetType(), static type =>
                {
#region AutoName
                    Span<Char> buffer = stackalloc Char[1024];
                    Int32 cursor = buffer.Length;
                    Boolean leaf = true;

                    Type? current = type;

                    while (current is not null && current.Assembly != Assembly)
                    {
                        if (!leaf && IsIgnored(current))
                        {
                        ...
                        {
                            Write(buffer, ref cursor, ".");
                        }

                        Type? parent = current.BaseType;

                        IgnoreParentAutoNameAttribute? ignore = GetIgnoreParentAttribute(current);

                        if (ignore is not null)
                        {
                            if (ignore.Parent is not null)
                            {
                                while (parent is not null && parent.Assembly != Assembly && parent != ignore.Parent)
                                {
                            ...
                            {
                                Int32 skips = ignore.Count > 0 ? ignore.Count : 1;

                                while (skips > 0 && parent is not null && parent.Assembly != Assembly)
                                {
                                    parent = parent.BaseType;
                                    skips--;
                                }
                        ...
                        while (parent is not null && parent.Assembly != Assembly && IsIgnored(parent))
                        {
                            parent = parent.BaseType;
                        }

                        Boolean root = IsRoot(parent);

                        if (root && leaf)
                        {
                             Write(buffer, ref cursor, nameof(Exception));
                             Write(buffer, ref cursor, ".");
                        }

                        ReadOnlySpan<Char> part = Get(current, parent, root);
                        Write(buffer, ref cursor, part);

                        current = parent;
                        leaf = false;
                    }

                    return new String(buffer.Slice(cursor));


                    [MethodImpl(MethodImplOptions.AggressiveInlining)]
                    static Boolean IsRoot(Type? type)
                    {
                        return type is null || type.Assembly == Assembly;
                    }

                    [MethodImpl(MethodImplOptions.AggressiveInlining)]
                    static Boolean IsIgnored(Type type)
                    {
                        return type.HasAttribute<IgnoreAutoNameAttribute>(false);
                    }

                    [MethodImpl(MethodImplOptions.AggressiveInlining)]
                    static IgnoreParentAutoNameAttribute? GetIgnoreParentAttribute(Type type)
                    {
                        return CustomAttributeExtensions.GetCustomAttribute<IgnoreParentAutoNameAttribute>(type, false);
                    }

                    static ReadOnlySpan<Char> Get(Type current, Type? parent, Boolean root)
                    {
                        ReadOnlySpan<Char> name = RemoveExceptionSuffix(current.Name);
                        ...
                        ReadOnlySpan<Char> basis = RemoveExceptionSuffix(parent!.Name);
                        Int32 common = FindCommonSequence(name, basis);
                        return name.Slice(common);
                    }

                    [MethodImpl(MethodImplOptions.AggressiveInlining)]
                    static ReadOnlySpan<Char> RemoveExceptionSuffix(ReadOnlySpan<Char> name)
                    {
                        Int32 index = MemoryExtensions.IndexOf(name, '`');
                        name = index >= 0 ? name.Slice(0, index) : name;

                        return name.EndsWith(nameof(Exception), StringComparison.Ordinal) ? name.Slice(0, name.Length - nameof(Exception).Length) : name;
                    }

                    [MethodImpl(MethodImplOptions.AggressiveInlining)]
                    static Int32 FindCommonSequence(ReadOnlySpan<Char> first, ReadOnlySpan<Char> second)
                    {
                        Int32 length = Math.Min(first.Length, second.Length);
                        Int32 count = 0;

                        while (count < length && first[count] == second[count])
                        {
                            count++;
                        }
                    ...
                    [MethodImpl(MethodImplOptions.AggressiveInlining)]
                    static void Write(Span<Char> buffer, ref Int32 cursor, ReadOnlySpan<Char> value)
                    {
                        cursor -= value.Length;
        ...
        IBusinessExceptionInfo IBusinessException.Info
        {
            get
            {
                return Info;
        ...
        IBusinessExceptionInfo IBusinessException.Business
        {
            get
            {
                return Business;
            ...
            : base(info, context)
        {
            ...
            RayId = info.GetValue<RayIdContext>(nameof(RayId));
            Name = info.GetString(nameof(Identity));
        ...
        public override void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            info.AddValue(nameof(RayId), RayId);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        Boolean IUnsafeTraceException.Set(RayIdContext context)
        {
            if (_context || !context)
            {
                return false;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Maybe<T> Get<T>(Object key)
        {
            return this[key].Unwrap(out Object? value) ? (T) value! : default(Maybe<T>);
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Add(KeyValuePair<Object, Object?> data)
        {
        ...
        public void Add(Object key, Object? value)
        {
            ...
            {
                throw new ArgumentAddingDuplicateWithKeyException<Object, TSR>(key, nameof(key));
            }
        ...
        public Maybe<Object?> this[Object key]
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
                ...
                return Data.Contains(key) ? Data[key] : default(Maybe<Object?>);
            }
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            set
            {
                ...
                if (value.Unwrap(out Object? item))
                {
                    Data[key] = item;
                    return;
                }
        ...
        protected virtual BusinessInfo ToBusinessInfo(Boolean include)
        {
            ...
            static BusinessInfo Inner(Exception exception, BusinessInfo inner, Boolean include)
            {
                ...
                if (exception is not BusinessException business)
                {
                    ...
                    {
                        ITraceException trace when include => new BusinessInfo(trace.Id, trace.RayId, exception.Message, null, null, trace.When, null, null, inner, null) { Business = false, Include = include, Data = exception.Data },
                        _ when include => new BusinessInfo(null, null, exception.Message, null, null, null, null, null, inner, null) { Business = false, Include = include, Data = exception.Data },
                        _ => inner
                    };
                }
            ...
            static BusinessInfo? Null(Exception exception, Boolean include)
            {
                return exception switch
                {
                    ...
                    ITraceException trace when include => new BusinessInfo(trace.Id, trace.RayId, exception.Message, null, null, trace.When, null, null, null) { Business = false, Include = include, Data = exception.Data },
                    _ when include => new BusinessInfo(null, null, exception.Message, null, null, null, null, null, null) { Business = false, Include = include, Data = exception.Data },
                    _ => null
                };
            }
        ...
        {
            return new BusinessInfo(Id, RayId, Message, Identity, Description, When, RetryAfter, Status, inner, ToBusinessInfoReason(include)) { Business = true, Include = include, Data = Data };
        }
        ...
        public record BusinessInfo : IBusinessExceptionInfo
        {
            [JsonIgnore, IgnoreDataMember, XmlIgnore, SoapIgnore]
            [System.Text.Json.Serialization.JsonIgnore]
            ...
            public Guid Id
            {
                ...
                init
                {
                    _id = value != default ? value : null;
                }
            }

            [JsonIgnore, IgnoreDataMember, XmlIgnore, SoapIgnore]
            [System.Text.Json.Serialization.JsonIgnore]
            private readonly RayIdContext? _ray;

            [JsonProperty(NullValueHandling = NullValueHandling.Ignore, DefaultValueHandling = DefaultValueHandling.Ignore, Order = 1)]
            ...
            [JsonProperty(PropertyName = nameof(Data), NullValueHandling = NullValueHandling.Ignore, Order = 6)]
            ...
            public IDictionary Data
            {
                ...
                init
                {
                    _data = value switch
                    {
                        ...
                        _ => DictionaryBaseUtilities.ToDictionary(value)
                    };
                }
            }
            ...
            [JsonProperty(DefaultValueHandling = DefaultValueHandling.Ignore, Order = 8)]
            [System.Text.Json.Serialization.JsonPropertyOrder(8)]
            [System.Text.Json.Serialization.JsonIgnore(Condition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingDefault)]
            public TimeSpan? RetryAfter { get; init; }

            [JsonProperty(NullValueHandling = NullValueHandling.Ignore, DefaultValueHandling = DefaultValueHandling.Ignore, Order = 9)]
            ...
            IBusinessExceptionInfo? IBusinessExceptionInfo.Inner
            {
                [MethodImpl(MethodImplOptions.AggressiveInlining)]
                get
                {
            ...
            ImmutableList<IBusinessExceptionInfo>? IBusinessExceptionInfo.Reason
            {
                get
                {
                    return Reason is not null ? ImmutableList.CreateRange<IBusinessExceptionInfo>(Reason) : null;
                }
            }
            ...
            public BusinessInfo(Guid? id, RayIdContext? context, String? message, String? identity, String? description, DateTime? when, TimeSpan? retry, HttpStatusCode? status, ImmutableList<BusinessInfo>? reason)
                : this(id, context, message, identity, description, when, retry, status, null, reason)
            {
            }

            public BusinessInfo(Guid? id, RayIdContext? context, String? message, String? identity, String? description, DateTime? when, TimeSpan? retry, HttpStatusCode? status, BusinessInfo? inner, ImmutableList<BusinessInfo>? reason)
            {
                _id = id;
                _ray = context;
                Message = message;
            ...
            {
                Deconstruct(out id, out context, out message, out name, out description, out when, out retry, out status, out _);
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public void Deconstruct(out Guid id, out RayIdContext context, out String? message, out String? name, out String? description, out DateTime? when, out TimeSpan? retry, out HttpStatusCode? status, out BusinessInfo? reason)
            {
                Deconstruct(out id, out context, out message, out name, out description, out when, out retry, out status, out reason, out _);
            }

            public void Deconstruct(out Guid id, out RayIdContext context, out String? message, out String? name, out String? description, out DateTime? when, out TimeSpan? retry, out HttpStatusCode? status, out BusinessInfo? reason, out ImmutableList<BusinessInfo>? inner)
            {
                id = Id;
                context = RayId;
                message = Message;
    ...
    [Serializable]
    public abstract class BusinessCustomException : BusinessStatusException
    {
        public abstract override String? Identity { get; init; }

        protected BusinessCustomException(HttpStatusCode status)
            : base(status)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status)
            : base(message, status)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status, Exception? exception)
            : base(message, status, exception)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status, BusinessException? exception)
            : base(message, status, exception)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status, params BusinessException?[]? inner)
            : base(message, status, inner)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status, Exception? exception, params BusinessException?[]? inner)
            : base(message, status, exception, inner)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status, BusinessException? exception, params BusinessException?[]? inner)
            : base(message, status, exception, inner)
        {
        }
        ...
        [System.ComponentModel.EditorBrowsable(System.ComponentModel.EditorBrowsableState.Never)]
#endif
        protected BusinessCustomException(SerializationInfo info, StreamingContext context)
            : base(info, context)
        {
        }
    }
    ...
    [Serializable]
    public abstract class BusinessCustomException<T> : BusinessStatusException<T>
    {
        public abstract override String? Identity { get; init; }

        protected BusinessCustomException(HttpStatusCode status, T code)
            : base(status, code)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status, T code)
            : base(message, status, code)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status, T code, Exception? exception)
            : base(message, status, code, exception)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status, T code, BusinessException? exception)
            : base(message, status, code, exception)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status, T code, params BusinessException?[]? inner)
            : base(message, status, code, inner)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status, T code, Exception? exception, params BusinessException?[]? inner)
            : base(message, status, code, exception, inner)
        {
        }

        protected BusinessCustomException(String? message, HttpStatusCode status, T code, BusinessException? exception, params BusinessException?[]? inner)
            : base(message, status, code, exception, inner)
        {
        }
