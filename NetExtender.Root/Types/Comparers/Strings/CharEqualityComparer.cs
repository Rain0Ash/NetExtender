...
namespace NetExtender.Types.Comparers
{
    public sealed class CharEqualityComparer : IEqualityComparer<Char>
    {
        ...
        private static CompareHandler GetComparerCore(StringComparison comparison)
        {
            return comparison switch
            {
                StringComparison.CurrentCulture => static (x, y) => x == y,
                StringComparison.CurrentCultureIgnoreCase => static (x, y) => Char.ToUpper(x, CultureInfo.CurrentCulture) == Char.ToUpper(y, CultureInfo.CurrentCulture),
                StringComparison.InvariantCulture => static (x, y) => x == y,
                StringComparison.InvariantCultureIgnoreCase => static (x, y) => Char.ToUpper(x, CultureInfo.InvariantCulture) == Char.ToUpper(y, CultureInfo.InvariantCulture),
                StringComparison.Ordinal => static (x, y) => x == y,
                StringComparison.OrdinalIgnoreCase => static (x, y) => Char.ToUpperInvariant(x) == Char.ToUpperInvariant(y),
                _ => throw new EnumUndefinedOrNotSupportedException<StringComparison>(comparison, nameof(comparison), null)
            };
        }
