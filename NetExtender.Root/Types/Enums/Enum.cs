...
namespace NetExtender.Types.Enums
{
    ...
    [System.Text.Json.Serialization.JsonConverter(typeof(Serialization.Json.EnumJsonConverter))]
    ...
    public partial class Enum<T> : IEnum<T>, IEquality<Enum<T>> where T : unmanaged, Enum
    {
        ...
        public String Name
        {
            ...
            get
            {
                return EnumUtilities.GetName(Id);
            }
        }
        ...
        [JsonProperty(PropertyName = nameof(Identifier), DefaultValueHandling = DefaultValueHandling.Ignore)]
        [System.Text.Json.Serialization.JsonPropertyName(nameof(Identifier))]
        [System.Text.Json.Serialization.JsonIgnore(Condition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingDefault)]
        public CultureIdentifier? Culture
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get
            {
        ...
        public virtual String ToString(String? format, IFormatProvider? provider)
        {
            ...
            {
                "d" or "D" or "g" or "G" or "x" or "X" or "f" or "F" => Id.ToString(format),
                "r" or "rid" or "r-id" or "R" or "RID" or "R-ID" => $"{GetType().Name} {{ {nameof(Id)} = {Id.ToString()}, {nameof(Title)} = {Title} }}",
                "rn" or "RN" or "rnumber" or "RNUMBER" or "r-number" or "R-NUMBER" => $"{GetType().Name} {{ {nameof(Id)} = {Id.ToString("D")}, Title = {Title} }}",
                "rt" or "RT" or "rtitle" or "RTITLE" or "r-title" or "R-TITLE" => $"{GetType().Name} {{ {nameof(Id)} = {ToTitle(Id)}, Title = {Title} }}",
                "id" or "ID" => Id.ToString(),
                "n" or "N" or "number" or "NUMBER" => Id.ToString("D"),
                "t" or "T" or "title" or "TITLE" => Title,
                "idt" or "IDT" or "idtitle" or "IDTITLE" or "id-title" or "ID-TITLE" => ToTitle(Id),
                _ => new StringBuilder(format).Replace("{TYPE}", GetType().Name).Replace("{ID}", Id.ToString()).Replace("{NUMBER}", Id.ToString("D")).Replace("{TITLE}", Title).Replace("{ID-TITLE}", ToTitle(Id)).ToString()
            };
        }
    }
}