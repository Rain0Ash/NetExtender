...
namespace NetExtender.Comparers.Common
{
    ...
    public sealed class CustomEqualityComparer<T> : IEqualityComparer<T>
    {
        ...
        public CustomEqualityComparer([NotNull] Func<T?, T?, Boolean> comparison)
        {
            Comparison = comparison ?? throw new ArgumentNullException(nameof(comparison));
        }
        
        public CustomEqualityComparer([NotNull] Func<T?, T?, Boolean> comparison, Func<T?, Int32> hash)
        {
            Comparison = comparison ?? throw new ArgumentNullException(nameof(comparison));
    ...
    public sealed class CustomEqualityComparer<T1, T2> : IEqualityComparer<T1, T2>
    {
        ...
        public CustomEqualityComparer([NotNull] Func<T1?, T2?, Boolean> comparison)
        {
            Comparison = comparison ?? throw new ArgumentNullException(nameof(comparison));
        }
        
        public CustomEqualityComparer([NotNull] Func<T1?, T2?, Boolean> comparison, Func<T1?, Int32> hash1, Func<T2?, Int32> hash2)
        {
            Comparison = comparison ?? throw new ArgumentNullException(nameof(comparison));
