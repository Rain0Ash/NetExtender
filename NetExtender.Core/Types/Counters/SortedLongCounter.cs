...
{
    public class SortedLongCounter<T> : ILongCounter<T>, IReadOnlyLongCounter<T> where T : notnull
    {
        private SortedDictionary<T, Int64> Internal { get; }
        ...
        public SortedLongCounter()
        {
            Internal = new SortedDictionary<T, Int64>();
        }

        public SortedLongCounter(IComparer<T>? comparer)
        {
            Internal = new SortedDictionary<T, Int64>(comparer);
        }

        public SortedLongCounter(IEnumerable<T> collection)
        {
        ...
        public SortedLongCounter(IEnumerable<T> collection, IComparer<T>? comparer)
        {
            if (collection is null)
            {
                throw new ArgumentNullException(nameof(collection));
