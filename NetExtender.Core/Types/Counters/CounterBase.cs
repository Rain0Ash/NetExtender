...
{
    public abstract class CounterBase<T, TCount> : CounterBase<TCount>, ICounter<T, TCount>, IReadOnlyCounter<T, TCount> where TCount : unmanaged, IConvertible
    {
    ...
    public abstract class CounterBase<TCount> where TCount : unmanaged, IConvertible
    {
        ...
        {
            return INetExtenderComparisonOperators<TCount>.GreaterThan(value, count);
        }

        protected virtual Boolean GreaterOrEquals(TCount value, TCount count)
        {
            return INetExtenderComparisonOperators<TCount>.GreaterThanOrEqual(value, count);
        }

        protected virtual TCount Increment(TCount value)
        {
            return INetExtenderStepOperators<TCount>.Increment(value);
        }

        protected virtual TCount Decrement(TCount value)
        {
            return INetExtenderStepOperators<TCount>.Decrement(value);
        }

        protected virtual TCount Add(TCount first, TCount second)
        {
            return INetExtenderAdditionOperators<TCount>.Addition(first, second);
        }

        protected virtual TCount Subtract(TCount first, TCount second)
        {
            return INetExtenderSubtractionOperators<TCount>.Subtraction(first, second);
        }
    }
}