...
#endif
    {
        ...
        public new sealed class OperatorHandler : INetExtenderOperator.OperatorHandler<TSelf, INetExtenderLogarithmicFunctions<TSelf>, INetExtenderLogarithmicFunctions<TSelf>.OperatorHandler.Set>
        {
            ...
            protected override IEnumerable<Result<INetExtenderOperator.OperatorHandler>> Initialize(Set set)
            {
                ...
                if (Set(in set.Log, Method<Func<TSelf, TSelf>>(this, Log)) is null)
                {
                    yield return Exception<Func<TSelf, TSelf>>(this, Log);
                }

                if (Set(in set.BaseLog, Method<Func<TSelf, TSelf, TSelf>>(this, Log)) is null)
                {
                    yield return Exception<Func<TSelf, TSelf, TSelf>>(this, Log);
                }

                Set(in set.LogP1) = Method(this, LogP1) ?? (static value => Log(Addition(value, One)));
                ...
                Set(in set.Log2P1) = Method(this, Log2P1) ?? (static value => Log2(Addition(value, One)));

                if (Set(in set.Log10, Method(this, Log10)) is null)
                {
                    yield return Exception(this, Log10);
                }

                Set(in set.Log10P1) = Method(this, Log10P1) ?? (static value => Log10(Addition(value, One)));
            }
        }
    }
}