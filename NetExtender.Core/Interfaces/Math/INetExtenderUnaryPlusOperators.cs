...
namespace NetExtender.Interfaces
{
...
#endif
    {
        ...
        public new const UnaryOperator Operator = UnaryOperator.UnaryPlus | UnaryOperator.Flags;
        ...
        public new sealed class OperatorHandler : INetExtenderOperator.OperatorHandler<TSelf, INetExtenderUnaryPlusOperators<TSelf, TResult>, INetExtenderUnaryPlusOperators<TSelf, TResult>.OperatorHandler.Set>
        {
            ...
            protected override IEnumerable<Result<INetExtenderOperator.OperatorHandler>> Initialize(Set set)
            {
                ...
                if (Set(in set.UnaryPlus, Operator<TResult>(@operator)) is null)
                {
                    yield return Exception<TResult>(@operator);
                }

                Set(in set.CheckedUnaryPlus) = Operator<TResult>(@operator | UnaryOperator.Checked) ?? set.UnaryPlus;
            }
        }
    }
}