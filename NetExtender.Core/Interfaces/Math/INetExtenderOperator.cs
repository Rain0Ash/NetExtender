...
namespace NetExtender.Interfaces
{
    ...
    public interface INetExtenderOperator<TSelf> : INetExtenderOperator
    {
        ...
        protected static THandler Operator<TOperator, THandler, TSet>() where TOperator : class, INetExtenderOperator<TSelf, TOperator, THandler, TSet> where THandler : OperatorHandler<TSelf, TOperator, TSet>, new() where TSet : OperatorHandler<TSelf, TOperator, TSet>.OperatorSet
        {
            return INetExtenderOperator<TSelf, TOperator, THandler, TSet>.Handler;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        protected static IUnaryReflectionOperator<TSelf, TResult>? Operator<TResult>(UnaryOperator @operator)
        {
            return Operator<TSelf, TResult>(@operator);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        protected static IBinaryReflectionOperator<TSelf, TOther, TResult>? Operator<TOther, TResult>(BinaryOperator @operator)
        {
            return Operator<TSelf, TOther, TResult>(@operator);
        }
    ...
    public interface INetExtenderOperator
    {
        ...
        public abstract class OperatorHandler<TSelf, TOperator, TSet> : OperatorHandler<TSelf, TOperator> where TOperator : class, INetExtenderOperator<TSelf, TOperator> where TSet : OperatorHandler<TSelf, TOperator, TSet>.OperatorSet
        {
            ...
            {
                return ReflectionUtilities.New<TSet>().Invoke();
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        protected static IUnaryReflectionOperator<TSelf, TResult>? Operator<TSelf, TResult>(UnaryOperator @operator)
        {
            return ReflectionOperator.Get<TSelf, TResult>(@operator);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        protected static IBinaryReflectionOperator<TSelf, TOther, TResult>? Operator<TSelf, TOther, TResult>(BinaryOperator @operator)
        {
        ...
        protected static Func<TResult>? Property<TResult>(OperatorHandler handler, String name)
        {
            if (name is null)
            {
            ...
            if (handler.Properties.Where(name, static (name, property) => property.HasName(name) && property.GetMethod is not null).ToArray() is not { Length: > 0 } properties)
            {
                return null;
            }
            ...
            {
                properties = properties.Where(name, static (name, method) => method.Name == name).ToArray();
                state = false;
                goto @new;
            }
        }
        ...
        protected static MethodInfo? Method(OperatorHandler handler, String name, Type[] parameters)
        {
            if (handler is null)
            {
                throw new ArgumentNullException(nameof(handler));
            ...
            if (handler.Generics.Where(name, static (name, method) => method.HasName(name)).ToArray() is not { Length: > 0 } methods)
            {
                return null;
            }
            ...
            {
                methods = methods.Where(name, static (name, method) => method.Name == name).ToArray();
                state = false;
                goto @new;
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        protected static TDelegate? Method<TDelegate>(OperatorHandler handler, TDelegate @delegate) where TDelegate : Delegate
        {
            if (handler is null)
            {
                throw new ArgumentNullException(nameof(handler));
            ...
            if (handler.Methods.Where(method.Name, static (name, method) => method.HasName(name)).ToArray() is not { Length: > 0 } methods)
            {
                return null;
            }
            ...
            {
                methods = methods.Where(method.Name, static (name, method) => method.Name == name).ToArray();
                state = false;
                goto @new;
            }
        }
