...
namespace NetExtender.Interfaces
{
...
#endif
    {
        ...
        public new const UnaryOperator Operator = UnaryOperator.UnaryNegation | UnaryOperator.Flags;
        ...
        public new sealed class OperatorHandler : INetExtenderOperator.OperatorHandler<TSelf, INetExtenderUnaryNegationOperators<TSelf, TResult>, INetExtenderUnaryNegationOperators<TSelf, TResult>.OperatorHandler.Set>
        {
            ...
            protected override IEnumerable<Result<INetExtenderOperator.OperatorHandler>> Initialize(Set set)
            {
                ...
                if (Set(in set.UnaryNegation, Operator<TResult>(@operator)) is null)
                {
                    yield return Exception<TResult>(@operator);
                }

                Set(in set.CheckedUnaryNegation) = Operator<TResult>(@operator | UnaryOperator.Checked) ?? set.UnaryNegation;
            }
        }
    }
}