...
namespace NetExtender.Utils.Serialization
{
    ...
    public static class XmlUtils
    {
        ...
        public static XmlNode CreateXPath(this XmlDocument document, String xpath)
        {
            if (document is null)
            {
                throw new ArgumentNullException(nameof(document));
            ...
            {
                throw new ArgumentException("Value cannot be null or empty.", nameof(xpath));
            }
            ...
            foreach (String part in xpath.Substring(1).Split('/'))
            {
                ...
                {
                    String? elName, attribute = null;
                    if (part.Contains("["))
                    {
                        part.SplitOnce("[", out elName, out attribute);
                    ...
                    {
                        throw new XmlException("Unsupported XPath attribute (missing @): " + part);
                    }

                    attribute.Substring(1).SplitOnce("='", out String? name, out String? value);
                    if (String.IsNullOrEmpty(value) || !value.EndsWith("'"))
                    {
                        throw new XmlException("Unsupported XPath attribute: " + part);
                    }
