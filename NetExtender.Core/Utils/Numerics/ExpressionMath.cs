...
namespace NetExtender.Utils.Numerics
{
    ...
    public static class MathGeneric<TValue, TResult>
    {
        ...
        static MathGeneric()
        {
            ConvertFunc = new Lazy<Func<TValue, TResult>>(() => ExpressionUtils.CreateExpression<TValue, TResult>(body => Expression.Convert(body, typeof(TResult))), true);
            AddFunc = new Lazy<Func<TResult, TValue, TResult>>(() => ExpressionUtils.CreateExpression<TResult, TValue, TResult>(Expression.Add, true), true);
            SubtractFunc = new Lazy<Func<TResult, TValue, TResult>>(() => ExpressionUtils.CreateExpression<TResult, TValue, TResult>(Expression.Subtract, true), true);
            MultiplyFunc = new Lazy<Func<TResult, TValue, TResult>>(() => ExpressionUtils.CreateExpression<TResult, TValue, TResult>(Expression.Multiply, true), true);
            DivideFunc = new Lazy<Func<TResult, TValue, TResult>>(() => ExpressionUtils.CreateExpression<TResult, TValue, TResult>(Expression.Divide, true), true);
            ModuloFunc = new Lazy<Func<TResult, TValue, TResult>>(() => ExpressionUtils.CreateExpression<TResult, TValue, TResult>(Expression.Modulo, true), true);
        }
    ...
    public static class MathGeneric<T>
    {
        static MathGeneric()
        {
            ...
            DivideFunc = new Lazy<Func<T, T, T>>(() => ExpressionUtils.CreateExpression<T, T, T>(Expression.Divide), true);
            ModuloFunc = new Lazy<Func<T, T, T>>(() => ExpressionUtils.CreateExpression<T, T, T>(Expression.Modulo), true);

            EqualFunc = new Lazy<Func<T, T, Boolean>>(() => ExpressionUtils.CreateExpression<T, T, Boolean>(Expression.Equal), true);
            NotEqualFunc = new Lazy<Func<T, T, Boolean>>(() => ExpressionUtils.CreateExpression<T, T, Boolean>(Expression.NotEqual), true);
            GreaterFunc = new Lazy<Func<T, T, Boolean>>(() => ExpressionUtils.CreateExpression<T, T, Boolean>(Expression.GreaterThan), true);
            GreaterEqualFunc = new Lazy<Func<T, T, Boolean>>(() => ExpressionUtils.CreateExpression<T, T, Boolean>(Expression.GreaterThanOrEqual), true);
            LessFunc = new Lazy<Func<T, T, Boolean>>(() => ExpressionUtils.CreateExpression<T, T, Boolean>(Expression.LessThan), true);
            LessEqualFunc = new Lazy<Func<T, T, Boolean>>(() => ExpressionUtils.CreateExpression<T, T, Boolean>(Expression.LessThanOrEqual), true);

            NegateFunc = new Lazy<Func<T, T>>(() => ExpressionUtils.CreateExpression<T, T>(Expression.Negate), true);
            AndFunc = new Lazy<Func<T, T, T>>(() => ExpressionUtils.CreateExpression<T, T, T>(Expression.And), true);
            OrFunc = new Lazy<Func<T, T, T>>(() => ExpressionUtils.CreateExpression<T, T, T>(Expression.Or), true);
