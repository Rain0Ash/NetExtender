...
namespace NetExtender.Types.Streams
{
    public class ThrottledStream : Stream
    {
        ...
        public ThrottledStream(Stream stream, Int32 speed, InformationSize size)
            : this(stream, size.ConvertToBytes(speed))
        {
        }
        
        public ThrottledStream(Stream stream, UInt64 speed, InformationSize size)
            : this(stream, size.ConvertToBytes(speed))
        {
        }

        public ThrottledStream(Stream stream, UInt64 speed = UInt64.MaxValue)
            : this(stream, speed, System.Reactive.Concurrency.Scheduler.Immediate)
        {
        }
        
        public ThrottledStream(Stream stream, UInt64 speed, IScheduler scheduler)
        {
            BaseStream = stream;
