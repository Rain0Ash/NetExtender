...
namespace NetExtender.Types.Concurrent.Counters
{
    public class ConcurrentLongCounter<T> : ILongCounter<T>, IReadOnlyLongCounter<T> where T : notnull
    {
        ...
        public ConcurrentLongCounter([NotNull] IEnumerable<T> collection, IEqualityComparer<T>? comparer)
        {
            if (collection is null)
            {
                throw new ArgumentNullException(nameof(collection));
        ...
        public ConcurrentLongCounter(Int32 concurrencyLevel, [NotNull] IEnumerable<T> collection, IEqualityComparer<T>? comparer)
        {
            if (collection is null)
            {
                throw new ArgumentNullException(nameof(collection));
        ...
        public Boolean TryGetValue([NotNull] T key, out Int64 value)
        {
            if (key is null)
            {
                throw new ArgumentNullException(nameof(key));
