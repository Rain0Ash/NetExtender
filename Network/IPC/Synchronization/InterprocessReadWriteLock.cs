...
namespace NetExtender.Network.IPC.Synchronization
{
    ...
    /// </summary>
    public class InterprocessReadWriteLock : IDisposable, IInterprocessReadWriteLock
    {
        ...
        public InterprocessReadWriteLock(String name, Int32 maxReaderCount)
        ...
        public InterprocessReadWriteLock(String name, Int32 maxReaderCount, TimeSpan waitTimeout)
        {
        ...
        /// <param name="waitTimeout">How long to wait before giving up aquiring read and write locks</param>
        public InterprocessReadWriteLock(Mutex mutex, Semaphore semaphore, Int32 maxReaderCount, TimeSpan waitTimeout)
        {
            if (maxReaderCount <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(maxReaderCount), @"Need at least one reader");
        ...
        {
            return new Semaphore(maxReaderCount, maxReaderCount, $"{nameof(InterprocessReadWriteLock)}_Semaphore_{name}");
        }
    }
}
