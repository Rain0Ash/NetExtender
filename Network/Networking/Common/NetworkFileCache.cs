...
namespace NetExtender.Network.Networking.Common
{
    ...
    {
        public delegate Boolean InsertHandler(NetworkFileCache cache, String key, Byte[] value, TimeSpan timeout);

        private readonly Dictionary<String, MemCacheEntry> _entriesByKey = new Dictionary<String, MemCacheEntry>();
        private readonly SortedDictionary<DateTime, String> _entriesByTimestamp = new SortedDictionary<DateTime, String>();

        private readonly Object _lock = new Object();
        private readonly SortedDictionary<String, FileCacheEntry> _pathsByKey = new SortedDictionary<String, FileCacheEntry>();
        private readonly SortedDictionary<DateTime, String> _pathsByTimestamp = new SortedDictionary<DateTime, String>();
        ...
        public Boolean Add(String key, Byte[] value, TimeSpan timeout = new TimeSpan())
        {
            lock (_lock)
            {
        ...
        /// <param name="timeout">Cache timeout (default is 0 - no timeout)</param>
        /// <returns>'true' if the cache value was added, 'false' if the given key was not added</returns>
        public Boolean Add(String key, String value, TimeSpan timeout = new TimeSpan())
        {
            lock (_lock)
            {
                // Try to find and remove the previous key
        ...
        public Tuple<Boolean, Byte[]> Find(String key)
        {
            lock (_lock)
            {
                // Try to find the given key
                return !_entriesByKey.TryGetValue(key, out MemCacheEntry cacheValue)
                    ? new Tuple<Boolean, Byte[]>(false, Array.Empty<Byte>())
                    : new Tuple<Boolean, Byte[]>(true, cacheValue.Value);
            }
        }
        ...
        /// <returns>'true' and cache value if the cache value was found, 'false' if the given key was not found</returns>
        public Tuple<Boolean, Byte[]> Find(String key, out DateTime timeout)
        {
            lock (_lock)
            {
                // Try to find the given key
                if (!_entriesByKey.TryGetValue(key, out MemCacheEntry cacheValue))
                {
                    timeout = new DateTime(0);
                    return new Tuple<Boolean, Byte[]>(false, Array.Empty<Byte>());
                }

                timeout = cacheValue.Timestamp + cacheValue.Timespan;
                return new Tuple<Boolean, Byte[]>(true, cacheValue.Value);
            }
        }
        ...
        public Boolean InsertPath(String path, String prefix = "/", TimeSpan timeout = new TimeSpan(), InsertHandler handler = null)
        {
        ...
        /// <param name="timeout">Cache timeout value</param>
        /// <returns>'true' if the cache path was found, 'false' if the given path was not found</returns>
        public Boolean FindPath(String path, out DateTime timeout)
        {
            lock (_lock)
            {
                // Try to find the given key
                if (!_pathsByKey.TryGetValue(path, out FileCacheEntry cacheValue))
                {
                    timeout = new DateTime(0);
                    return false;
                }
        ...
        public void Watchdog(DateTime utc)
        {
            ...
            {
                KeyValuePair<DateTime, String> entry = _entriesByTimestamp.First();
                if (!_entriesByKey.TryGetValue(entry.Value, out MemCacheEntry cachedValue))
                {
                    break;
                }
            ...
            {
                KeyValuePair<DateTime, String> entry = _pathsByTimestamp.First();
                if (!_pathsByKey.TryGetValue(entry.Value, out FileCacheEntry cachedValue))
                {
                    break;
                }
                ...
                if (cachedValue.Timestamp + cachedValue.Timespan <= utc)
                {
                    ...
                    String prefix = cachedValue.Prefix;
                    TimeSpan timespan = cachedValue.Timespan;
                    InsertHandler handler = cachedValue.Handler;
        ...
        private Boolean InsertPathInternal(String path, String prefix, TimeSpan timeout, InsertHandler handler)
        {
            try
            {
                ...
                if (Directory.GetDirectories(path)
                    .Select(item => new {item, key = keyPrefix + HttpUtility.UrlDecode(Path.GetFileName(item))})
                    .SelectWhere(pair => !InsertPathInternal(pair.item, pair.key, timeout, handler), pair => pair.item)
                    .Any())
                {
                    return false;
                }
        ...
        private Boolean RemovePathInternal(String path)
        {
            lock (_lock)
            {
        ...
        private readonly struct MemCacheEntry
        {
        ...
        private readonly struct FileCacheEntry
        {
