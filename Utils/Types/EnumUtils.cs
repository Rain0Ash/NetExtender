...
namespace NetExtender.Utils.Types
{
    public static class EnumUtils
    {
        ...
        {
            return CacheValues<T>.IsEmpty ? (T?) null : CacheMinMaxValues<T>.MinValue;
        }
        ...
        {
            return CacheValues<T>.IsEmpty ? (T?) null : CacheMinMaxValues<T>.MaxValue;
        }
        ...
        /// <summary>
        ///     Gets the <see cref="Google.Protobuf.WellKnownTypes.Value" /> of specified enumration member.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="value"></param>
        ...
        {
            EnumMemberAttribute attr = value.ToMember().EnumMemberAttribute;
            if (attr is not null)
            {
                member = attr.Value;
                return true;
        ...
        public static Boolean TryGetLabel<T>(this EnumMember<T> member, Int32 index, out String label) where T : unmanaged, Enum
        {
            ...
            return member.Labels.TryGetValue(index, out label);
        }
        ...
        {
            public static readonly ReadOnlyArray<EnumMember<T>> Members;

            static CacheMembers()
            {
                Members = CacheNames<T>.Names.Select(x => new EnumMember<T>(x)).ToReadOnlyArray();
            }
        }
        ...
        {
            public static readonly FrozenDictionary<T, IReadOnlyList<TAttribute>> Cache;

            static CacheAttributes()
            {
                Cache = GetValues<T>().ToFrozenDictionary(x => x, x => x.ToMember().FieldInfo
        ...
        {
            public static readonly Boolean IsFlags;
        ...
        {
            public static readonly FrozenStringKeyDictionary<EnumMember<T>> MemberByName;

            static CacheMembersByName()
            {
                MemberByName = CacheMembers<T>.Members.ToFrozenStringKeyDictionary(x => x.Name);
            }
        }
