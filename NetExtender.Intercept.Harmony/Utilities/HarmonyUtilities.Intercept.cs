...
namespace NetExtender.Harmony.Utilities
{
    public static partial class InterceptHarmonyUtilities
    {
        ...
        public static class Intercept<T, THandler> where T : class, IInterceptIdentifierTarget<T> where THandler : struct
        {
            ...
            {
                if (HarmonyUtilities.InstantiationMemory.New is not { } @new || @new != typeof(T) || HarmonyUtilities.InstantiationMemory.Old is not { } old || HarmonyUtilities.InstantiationMemory.Method is not { } method)
                {
                ...
                foreach (CodeInstruction instruction in instructions)
                {
                    ...
                    constructor = HarmonyUtilities.FindConstructorForInstantiation(@new, constructor) ?? throw new NeverOperationException();
            ...
            public static MethodInfo? Apply(HarmonyLib.Harmony harmony, MethodBase original, THandler handler)
            {
                ...
                return HarmonyUtilities.ChangeInstantiation(harmony, original, Transpilers.HttpClientTranspiler, typeof(T).BaseType.BaseType, typeof(T));
            }
