...
namespace NetExtender.Harmony.Utilities
{
    public static partial class InterceptHarmonyUtilities
    {
        ...
        public abstract class InterceptHarmonyStreamWriter : InterceptStreamWriter, IInterceptIdentifierTarget<InterceptHarmonyStreamWriter>
        {
            ...
            public InterceptHarmonyStreamWriter(Stream stream)
                : base(stream)
            {
                Initialize();
            }

            public InterceptHarmonyStreamWriter(Stream stream, Encoding encoding)
                : base(stream, encoding)
            {
                Initialize();
            }

            public InterceptHarmonyStreamWriter(Stream stream, Encoding encoding, Int32 bufferSize)
                : base(stream, encoding, bufferSize)
            {
                Initialize();
            }

            public InterceptHarmonyStreamWriter(Stream stream, Encoding? encoding = null, Int32 bufferSize = -1, Boolean leaveOpen = false)
                : base(stream, encoding, bufferSize, leaveOpen)
            {
                Initialize();
            }

            public InterceptHarmonyStreamWriter(String path)
                : base(path)
            {
                Initialize();
            }

            public InterceptHarmonyStreamWriter(String path, FileStreamOptions options)
                : base(path, options)
            {
                Initialize();
            }

            public InterceptHarmonyStreamWriter(String path, Boolean append)
                : base(path, append)
            {
                Initialize();
            }

            public InterceptHarmonyStreamWriter(String path, Boolean append, Encoding encoding)
                : base(path, append, encoding)
            {
                Initialize();
            }

            public InterceptHarmonyStreamWriter(String path, Boolean append, Encoding encoding, Int32 bufferSize)
                : base(path, append, encoding, bufferSize)
            {
                Initialize();
            }

            public InterceptHarmonyStreamWriter(String path, Encoding encoding, FileStreamOptions options)
