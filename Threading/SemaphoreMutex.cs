...
namespace NetExtender.Threading
{
    public sealed class SemaphoreMutex : IDisposable
    {
        ...
        public SemaphoreMutex(Boolean initiallyOwned, [CanBeNull] String? name)
            : this(new Mutex(initiallyOwned, name))
        {
        }

        public SemaphoreMutex(Boolean initiallyOwned, [CanBeNull] String? name, out Boolean createdNew)
            : this(new Mutex(initiallyOwned, name, out createdNew))
        {
        }

        private SemaphoreMutex([NotNull] Mutex mutex)
        {
        ...
        {
            return Mutex?.GetAccessControl() ?? throw new ObjectDisposedException(nameof(Mutex));
        }

        public void SetAccessControl([NotNull] MutexSecurity security)
        {
            if (Mutex is null)
            {
                throw new ObjectDisposedException(nameof(Mutex));
