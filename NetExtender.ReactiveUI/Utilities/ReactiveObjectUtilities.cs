...
namespace NetExtender.ReactiveUI.Utilities
{
    public static class ReactiveObjectUtilities
    {
        ...
        public static PropertySubnotifier<T> Register<T, TProperty>(this T value, Expression<Func<T, TProperty>> when, params Expression<Func<T, TProperty>>?[]? properties) where T : ReactiveObject
        {
            if (when is null)
            {
